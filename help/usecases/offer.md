...
従業員の内定通知は、従業員が組織で最初に体験する通知の1つです。 その結果、オファーレターがブランドであることを確認したいが、毎回ワープロで最初から文字を作成する必要はない。[!DNL Adobe Acrobat Services] APIを使用すると、[新しい従業員へのオファーレターの作成と配信](https://developer.adobe.com/document-services/use-cases/agreements-and-contracts/employee-offer-letters)の主要な部分を迅速、簡単、かつ効果的に処理できます。
...
* [プロジェクトサンプル](https://developer.adobe.com/document-services/use-cases/agreements-and-contracts/employee-offer-letters)
...
OfferLetter-Templateで、新しい&#x200B;**Document Generation**&#x200B;ボタンをクリックします。 サイドパネルが開きます。 「**今すぐ始める**」をクリックします。 サンプルのJSONデータに貼り付けるテキスト領域が表示されます。 JSONの「offer-data」スニペットを上記からテキスト領域にコピーします。 次のように表示されます。
...
[!DNL Adobe Acrobat Services]は6か月間無料で使用でき、その後[従量課金制](https://developer.adobe.com/document-services/pricing/main)で文書トランザクション1件につきわずか0.05 USDで利用できます。ビジネスの成長に合わせてオファーレターのワークフローを拡張できます。 [を開始するには](https://www.adobe.io/apis/documentcloud/dcsdk/gettingstarted.html)
